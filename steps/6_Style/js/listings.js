function buildListings(tripSet, pop) {
  ...
  filteredTripSet.forEach( (listing, index) => {
    buildListing(columns[index % columns.length], listing, pop)
  })
}

function buildListing(container, listing, pop) {
  const listingElm = document.createElement('article')
  listingElm.classList.add("listing")
  if (pop) listingElm.classList.add("pop")
